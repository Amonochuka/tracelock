...
- `src/app/admin/(dashboard)/users/page.tsx` — Personnel list with **Create User** modal. Submits to `POST /admin/users` and instantly refreshes the table on success. Operator names link to their activity drill-down, and the per-row `⋮` menu offers Change Role (`PUT /admin/users/{id}/role`), Manage Access (`GET /users/{id}/access` + `POST`/`DELETE /admin/access`), Manage Credentials (enrol/revoke), Unlock Account, and Delete User.
