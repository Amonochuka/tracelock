...
- `src/app/admin/(dashboard)/users/page.tsx` — Personnel list with **Create User** modal. Submits to `POST /admin/users` and instantly refreshes the table on success. Operator names link to their activity drill-down, and the per-row `⋮` menu offers Change Role (`PUT /admin/users/{id}/role`), Manage Access (`GET /users/{id}/access` + `POST`/`DELETE /admin/access`), Manage Credentials (enrol/revoke), **View Access History** (paginated `GET /users/{id}/events` with zone names resolved from `/zones` + `/users/{id}/access`), Unlock Account, and Delete User.
- `src/app/admin/(dashboard)/users/[id]/page.tsx` — Per-operator activity record. Aggregates from `GET /admin/users/{id}/analytics`, a Recharts zone-breakdown chart, and paginated history via `GET /admin/users/{id}/events`.
...
### 2026-08-29 — Per-User Access History (modal), Access-Token Refresh, and Simulator Event Pagination

- **Status:** Complete
- **Summary:** Closed three reported gaps. (1) **Per-user access history modal**: the backend `GET /users/{id}/events` and `/users/{id}/access` endpoints existed but had no UI; the Users page gained a **View Access History** action in each row's `⋮` menu that opens a modal with the operator's currently granted zones plus a paginated (12/page) audit trail. Zone names are resolved from `/zones` plus `/users/{id}/access` so revoked grants still render a readable name. **(Note:** after rebase, an 08-24 entry's dedicated `/admin/users/[id]` drill-down page also landed; the modal remains as a lightweight inline alternative, the row-menu entries coexist.) (2) **Access-token refresh**: the frontend previously stored only the 15-minute access token and logged out on expiry. The app now also persists the backend-issued `refresh_token` on login/bootstrap, best-effort revokes it on logout (`POST /logout`), and schedules a proactive `POST /refresh` ~60 seconds before the access token's `exp` claim (decoded client-side) — re-fetching `/me` afterward so administrator role changes propagate to the live session. If the refresh token is expired/revoked/unreachable, the user is signed out cleanly. (3) **Simulator event pagination**: the Hash Chain Events log was hard-capped at 15 rows; it now pages through the full `GET /zones/{id}/events?limit=15&offset=N` response with Previous/Next controls, keeps the page on manual refresh and after a simulated scan, and resets to page 1 when the zone selector changes.
- **Files touched:** `src/app/admin/(dashboard)/users/page.tsx`, `src/context/AuthContext.tsx`, `src/components/SignInForm.tsx`, `src/app/bootstrap/page.tsx`, `src/app/admin/(dashboard)/simulator/page.tsx`, `FRONTEND_PROGRESS_REPORT.md`.
- **Validation:** `npm run build` exit code 0, TypeScript clean; rebased onto the 08-24 changes with the shared `users/page.tsx` and report conflicts resolved. ESLint on the changed files reports no new problems versus baseline.
...
- **Correction:** end-to-end verification exposed that the `ListUsers` repo query never selected `locked_until`, so the API omitted lock state and the LOCKED badge / Unlock Account action never rendered. Fixed by adding the column to the query and scan; verified via a live lock → API response → unlock cycle on a throwaway user (deleted afterwards).
